...
export class Room {
     ...
     */
    constructor(userId, doc, provider, name, key) {
        /**
 ...
 * @return {Room}
 */
const openRoom = (userId, doc, provider, name, key) => {
    // there must only be one room
    if (rooms.has(name)) {
        throw error.create(`A Yjs Doc connected to room "${name}" already exists!`)
    }
    const room = new Room(userId, doc, provider, name, key)
...
export class WebrtcProvider extends Observable {
    ...
    constructor(
        ...
        this.room = null
        this.key.then(key => {
            this.room = openRoom(userId, doc, this, roomName, key)
